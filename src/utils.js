...
exports.writeOnExistingFile = (config, req) => {
  try {
    ...
    if (typeof req.description === 'object') {
      description = JSON.stringify(req.description, null, 2);
    }
...
exports.loadFile = (basePath, jsonOrFile) => {
  let filePath = `${basePath}/${jsonOrFile}`;
  if (!exists(filePath)) return jsonOrFile;
  try {
...
exports.loadYAMLOrParse = (basePath, yamlOrFile) => {
  ...
  if (!exists(filePath)) return parseJsonSafe(yamlOrFile);
  try {
